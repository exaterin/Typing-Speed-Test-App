...
public class TextDisplayManager {
    ...
    public void updateTextStyles(String textEntered) {
        ...
        int len = textEntered.length() - usedWordsCount;
        String originalText = textPane.getText();

        for (int i = 0; i < len; i++) {

            int ind_entered = i + usedWordsCount + emptySpaceCount;
            if (i < originalText.length() && textEntered.charAt(ind_entered) == originalText.charAt(i)) {
